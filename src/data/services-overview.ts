import { navigationContent, type DropdownItem } from "./navigation";
...
/**
 * The full service catalog, sourced from the Services nav dropdown so the
 * overview page and the mega-menu never drift. Each entry is
 * `{ name, href, icon, description, group }`.
 */
export const serviceCatalog: DropdownItem[] =
...
export const servicesOverviewContent: ServicesOverviewContent = {
  hero: {
    ...
    pills: [
      `${serviceCatalog.length} specialist offers`,
